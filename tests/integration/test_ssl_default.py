...
def _ssl_cipher(conn):
    ...
    cur = conn.cursor()
    cur.execute("SELECT VARIABLE_VALUE FROM information_schema.session_status "
...
class TestSslDefault(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        c = mariadb.connect(**_base_conf(ssl=False))
        cur = c.cursor()
        cur.execute("SELECT @@have_ssl")
        cls._server_has_ssl = (cur.fetchone()[0] == 'YES')
        c.close()
